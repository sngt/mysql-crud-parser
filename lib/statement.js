...
// TODO Support various patterns - UNION, JOIN, etc.
class Select extends Statement {
    constructor(tokens) {
        ...
        if (tokens.firstExpression().toUpperCase() === 'WHERE') {
            const whereTokens = tokens.shiftUntilExpressionPattern(/^(GROUP|ORDER|LIMIT)$/i);
            this.where = new Where(whereTokens.hasEntry() ? whereTokens : tokens);
        }
        if (tokens.firstExpression().toUpperCase() === 'GROUP') {
            tokens.shift();// GROUP
            if (tokens.firstExpression().toUpperCase() !== 'BY') {
                throw new Error('You have an error in your SELECT SQL syntax near "' + tokens.stringify() + '".');
            }
            tokens.shift();// BY
            const groupsTokens = tokens.shiftUntilExpressionPattern(/^(ORDER|LIMIT)$/i);
            this.group = (groupsTokens.hasEntry() ? groupsTokens : tokens).splitByExpressionPattern(/^,$/).map((groupTokens) => {
        ...
        if (tokens.firstExpression().toUpperCase() === 'ORDER') {
            tokens.shift();// ORDER
            if (tokens.firstExpression().toUpperCase() !== 'BY') {
                throw new Error('You have an error in your SELECT SQL syntax near "' + tokens.stringify() + '".');
            }
            tokens.shift();// BY
            const ordersTokens = tokens.shiftUntilExpressionPattern(/^LIMIT$/i);
            this.order = (ordersTokens.hasEntry() ? ordersTokens : tokens).splitByExpressionPattern(/^,$/).map((orderTokens) => {
    ...
    toString(option) {
        ...
        let sql = option.processReservedWords('SELECT ') + this.cols.map(option.processSchemaName).join(',');
        if (this.table) {
...
class Where {
    constructor(tokens) {
        ...
        tokens.splitByExpressionPattern(/^(AND|OR)$/i).forEach((conditionTokens) => {
            ...
            if (tokensBuffer.countExpressionPattern(/^\($/) !== tokensBuffer.countExpressionPattern(/^\)$/)) {
                return;
            }
    ...
    toString(option) {
        ...
        option = option || Option.DEFAULT;
        return option.processReservedWords('WHERE ') +
...
class WhereCondition {
    ...
    constructor(tokens) {
        ...
        if (this.operator.match(/BETWEEN$/i)) {
            ...
            tokens.shift();// AND
            this.value = {min: minTokens.stringify(), max: tokens.stringify()};
    ...
    toString(option) {
        option = option || Option.DEFAULT;
        let condition = this.linkType ? option.processReservedWords(this.linkType) + ' ' : '';
        if (this.col) {
