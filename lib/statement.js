...
class Select extends Statement {
    constructor(tokens) {
        ...
        if (tokens.firstExpression().toUpperCase() === 'GROUP') {
            ...
            this.group = (
                groupsTokens.hasEntry()
                ? groupsTokens
                : tokens).splitByExpressionPattern(/^,$/).map((groupTokens) => {
        ...
        if (tokens.firstExpression().toUpperCase() === 'ORDER') {
            ...
            this.order = (
                ordersTokens.hasEntry()
                ? ordersTokens
                : tokens).splitByExpressionPattern(/^,$/).map((orderTokens) => {
...
class Where {
    ...
    toString(option) {
        ...
        option = option || ToStringOption.DEFAULT;
        return option.processReservedWords('WHERE ') + this.conditions.map((condition) => {
